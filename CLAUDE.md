...
- `workspace.db` — SQLite (better-sqlite3): таблицы `projects`, `users` (логины/роли/права), `chat_sessions` (+ `user_id` — владелец чата) + `chat_messages` (история чатов, **приватная по пользователю**, чистится по ретенции), `tg_sessions` (привязки Telegram).
...
- `chat.ts` — серверные чат-сессии браузера (`Map<chatId, ChatSession>`, источник истины). WS-хендлеры `chat_create` / `chat_send` / `chat_cancel` / `chat_reset` / `chat_close`. История и `session_id` персистятся в `workspace.db` (`chat_messages` + `chat_sessions`) — диалог переживает и переподключение (`chat_restore`), и рестарт бэкенда (поднимается из БД). **Приватность:** каждый чат принадлежит пользователю (`chat_sessions.user_id`); все WS-хендлеры проверяют владельца (`owns(s)`), `listChats(wid, userId)` отдаёт только свои чаты. Легаси-чаты с `user_id IS NULL` (созданные до приватности) — общие, видны всем; новый чат закрепляется за создателем. Ретенция: `pruneOldMessages()` чистит сообщения старше `CHAT_RETENTION_MS` (дефолт 7 дн.) при старте и по таймеру `CHAT_RETENTION_SWEEP_MS` (дефолт 1 ч). GC отвязанных живых сессий через `DETACH_GC_MS`=30 мин. `listChats(wid, userId)` (память + персист с непустой историей) → `/api/workspaces/:wid/chats`.
...
⚠️ **Две разные «роли», не путать.** (1) *Тип агента* (`manager`/`overseer`) — где работает `claude`. (2) *Роль пользователя* (`sysadmin`/`coder`/`analyst`/`tester`/`tourist`) — что пользователю можно.

⚠️ **Права теперь пер-пользовательские, а не чисто ролевые.** У каждого юзера в `users.permissions` лежит **свой** JSON-массив `Action`. Роль — лишь (а) *шаблон* прав при создании (засев из матрицы), (б) подпись в UI, (в) ролевая надстройка промпта агента. Гейтинг проверяет именно `permissions` пользователя (`userCan`), а не роль. Сисадмин правит права чекбоксами на странице «Пользователи» (`UsersAdmin.tsx`), пер-юзерно.

- **`auth.ts`** — пользователи в `workspace.db` (`users`: + `first_name`/`last_name`/`email`/`permissions`), пароли `scrypt`, сессия — HMAC-токен `{id,exp}` в httpOnly-cookie. Роуты `/api/auth/login` / `logout` / `me` + CRUD `/api/auth/users` (`GET`/`POST`/`PATCH`/`DELETE` — только право `user.manage`). `requireAuth`, `userFromCookieHeader` (для апгрейда WS), `reqUser(req)`. `ensureUserManager()` на старте: если ни у кого нет `user.manage` — повышает bootstrap-админа до `sysadmin` (защита от локаута). `BOOTSTRAP_ADMIN` без роли → `sysadmin`.
- **`permissions.ts`** — `Action`/`ACTIONS`, матрица-шаблон `defaultPermissions(role)`, `can(role, action)` (по шаблону) и **`userCan(permissions, action)`** (основной гейт). Действия: `agent.run`, `terminal.open`, `git.commit`, `git.push`, `project.add`, `project.delete`, `chat.read`, `user.manage`. Шаблоны ролей: `sysadmin` (всё, в т.ч. управление юзерами), `coder` (всё кроме `user.manage`), `analyst` (агент + чтение), `tester` (агент + терминал + чтение), `tourist` (только чтение).
- **Гейт.** Backend: `/api/*` (кроме `/api/auth/*`) за `requireAuth`; чувствительные роуты — за `gate(action)` → `userCan`; WS — пользователь из cookie (`wsUsers`), `chat_*` требует `agent.run`, `terminal_create` — `terminal.open` (тоже `userCan`). Frontend — **источник истины всё равно сервер**, кнопки скрываются по `userCan(user, action)` (`user.permissions`; зеркало шаблонов в `frontend/src/auth.ts`).
- **Скиллы под роли.** `backend/skills/role-{coder,analyst,tester,tourist}/SKILL.md` синкаются (`syncManagerSkills`); `sysadmin` использует надстройку как у `coder`. Ролевая надстройка промпта — `ROLE_NOTES` (`agents.ts`) → `runClaude({roleNote})`, роль приходит из `wsUsers` через `handleChatWs`.
